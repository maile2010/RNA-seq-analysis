...
# visualize the differentially expressed genes for males and females in a volcano plot
...
EnhancedVolcano(subset(network_dataset, select=c(19,20,1,4,5)), title = "DCM vs NF", 
                lab = network_dataset$uniprot_gn_symbol, labSize = 3, x = 'logFC', 
...
EnhancedVolcano(subset(network_dataset, select=c(19,20,7,10,11)), title = "HCM vs NF", 
                lab = network_dataset$uniprot_gn_symbol, labSize = 3, x = 'logFC_2', 
...
#filter cleaned DEGs  which overlap with target_HCM
...
#Venn diagram to find overlap between DEGs and DEM targets
venn.diagram(
